...
function google_Auth() {
  ...
  //returns response from request to authorize an action -> plays auth, action, returns response as promise
      return CREDENTIALS.then( (credsResolved) => { //"then" returns resolved version of promise to first arg i.e. credsResolved
              
        const {client_secret, client_id, redirect_uris} = credsResolved.installed
  ...
  function _downloadFile(id, auth) {
    ...
    return new Promise( (resolve, reject) => { //will eventually return an array of promises representing each piece of the file
